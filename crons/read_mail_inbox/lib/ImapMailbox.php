...
class ImapMailbox {
    ...
    public function __construct($imapPath, $login, $password, $attachmentsDir = null, $serverEncoding = 'utf-8', $conn) {
    ...
    public function getMail($mailId) {
        ...
        if (empty($mailStructure->parts)) {
            ...
            $match_subject = strtolower('Summary of Automated Recurring Billing');
			$mail_subject = strtolower($mail->subject);
			
            if ($mail_subject == $match_subject) {
				
				//echo $mail_subject; exit;
                // Create record in DB to get it's id
                $sql_ins = "INSERT INTO tbl_user_recurring_billing_file (pk_user_recurring_billing_file_id) VALUES(NULL)";
			   
			   /* mysqli_query($this->conn, $sql_ins) or die(mysqli_error());
                $id = mysqli_insert_id($this->conn);*/
				
                $conn = $this->conn;
			  	$date_res = $conn->query($sql_ins);
				
				//$id = $conn->lastInsertId();
			    $id = mysqli_insert_id($conn);
			   
			    $_SESSION['authorize_net_csv_file_id'] = $id;
                //echo $id; exit;

                foreach ($mailStructure->parts as $partNum => $partStructure) {
                    $this->initMailPart($mail, $partStructure, $partNum + 1, $id);
                }
            }
        }
    ...
    protected function initMailPart(IncomingMail $mail, $partStructure, $partNum, $id = 0) {
        ...
        if ($attachmentId) {
            ...
            if ($this->attachmentsDir) {
                ...
                $fileSysName = strtolower($fileName);
                $file_name = $fileSysName;

                $allowed_files = array('successful.csv', 'failed.csv');

                // Save only Authhorize.Net CSV files with subject.
                if (in_array($fileSysName, $allowed_files)) {

                    $mail_id = $mail->id;
                    $mail_date = $mail->date;
                    $mail_date_parts = explode(' ', $mail_date);
                    //echo $mail_date; exit;

                    $fileSysName = $mail_id . '_' . $mail_date_parts[0] . '_' . $fileSysName;

                    $attachment->filePath = $this->attachmentsDir . DIRECTORY_SEPARATOR . $fileSysName;
                    file_put_contents($attachment->filePath, $data);
                    $mail->attachmentsList[$partNum] = $attachment->filePath;

                    // Update tbl_user_recurring_billing_file table
                    $update_sql = "UPDATE tbl_user_recurring_billing_file SET user_recurring_billing_file_email_inbox_date='" . $mail_date . "'";

                    if ($file_name == 'successful.csv') {
                        $update_sql .= " ,user_recurring_billing_file_successful_file = '" . $fileSysName . "'";
                    } else if ($file_name == 'failed.csv') {
                        $update_sql .= " ,user_recurring_billing_file_failed_file = '" . $fileSysName . "'";
                    }

                    $update_sql .= " ,user_recurring_billing_file_import_status='No', user_recurring_billing_file_created = NOW() WHERE pk_user_recurring_billing_file_id = $id";
                    //print_r($this->conn);exit;
                    mysqli_query($this->conn, $update_sql) or die(mysqli_error($this->conn));
                }
            }
